...
# CSS RECONSTRUIDO PARA EVITAR ERRORES DE RENDERIZADO
st.markdown("""
    <style>
    .main { background-color: #0E1117; }
    
    /* Logo y Cabecera Sidebar */
    ...
    /* MENU LATERAL: LIMPIO Y SIN BOLITAS VISIBLES */
    ...
    /* Ocultar el radio button nativo pero mantener el texto */
    ...
    /* Resaltado al seleccionar o pasar el ratón */
...
with st.sidebar:
    ...
    menu = st.radio(
        "SISTEMA",
        ["🏠 DASHBOARD", "🤖 FLOTA DE BOTS", "📜 EL GRIMORIO"],
        label_visibility="collapsed"
    )
    
    st.markdown("---")
    st.success("LABORATORIO ONLINE")

# --- 4. CONTENIDO ---

if df_all.empty:
    st.info("Sincronizando con Irkalla...")
else:
    if menu == "🏠 DASHBOARD":
        ...
        fig_donut = px.pie(bot_stats, values=abs(bot_stats['net_profit']), names='bot_name', hole=0.5, height=400)
